...
class WebSocketPathHandler(val transceiver: WebSocketTransceiver, private val channelIds: AtomicLong, private vararg val webSocketPaths: String): ChannelInboundHandlerAdapter() {
    ...
    override fun channelRead(ctx: ChannelHandlerContext, msg: Any) {
        ...
        try {
            if (isConfiguredWebSocketPath(path)) {
                ...
                ctx.pipeline().addAfter(ctx.name(), WebSocketServerProtocolHandler::class.java.name,
                        WebSocketServerProtocolHandler(path, null, true))
                ctx.pipeline().addAfter(WebSocketServerProtocolHandler::class.java.name, "frameHandler",
                        WebSocketFrameHandler(channelIds.incrementAndGet(), transceiver, path))
                ctx.pipeline().remove(WebSocketPathHandler::class.java.name)
                logger.info("WebSocketServerProtocolHandler and WebSocketFrameHandler  added for websocket path: {}", path)
