...
class ProtobufServer : Server(), ServerTransmitter<KonemPMessage>, ProtobufChannelReader {
  ...
  override fun addChannel(port: Int, vararg args: String): Boolean {
    ...
    return addChannel(port, transceiver)
  ...
  override fun broadcastOnChannel(port: Int, message: KonemPMessage, vararg args: String) {
    val transceiver = getTransceiverMap()[port] as ProtobufTransceiver
    transceiver.broadcastMessage(message)
  }

  override fun broadcastOnAllChannels(message: KonemPMessage, vararg args: String) {
  ...
  override fun sendMessage(addr: InetSocketAddress, message: KonemPMessage) {
